...
(function initSidebarResize(){
  ...
  const savedW = localStorage.getItem('fantasy_sidebar_w');
  ...
  handle.addEventListener('pointerup', e => {
    ...
    localStorage.setItem('fantasy_sidebar_w', Math.round(sidebar.getBoundingClientRect().width));
  });
})();

/* ================================================================
   SETTINGS PANEL — sizes, font scale, glow, themes
================================================================ */
const ALL_THEMES = [
  { key: 'default',  label: 'Savaş',  swatch: '#c89a4c' },
  { key: 'warrior',  label: 'Savaşçı', swatch: '#c2513f' },
  { key: 'druid',    label: 'Druid',  swatch: '#6f9e46' },
  { key: 'mage',     label: 'Büyücü', swatch: '#4d7fc4' },
  { key: 'paladin',  label: 'Paladin', swatch: '#cbaa4c' },
  { key: 'rogue',    label: 'Hırsız', swatch: '#8b8f96' },
  { key: 'warlock',  label: 'Warlock', swatch: '#8f56c4' },
  { key: 'priest',   label: 'Rahip',  swatch: '#d488a8' },
];

const DEFAULT_SETTINGS = { itemW: 68, libW: 58, fontScale: 100, glow: false, theme: 'default' };

function loadSettings() {
  try {
    const raw = localStorage.getItem('fantasy_settings');
    return raw ? { ...DEFAULT_SETTINGS, ...JSON.parse(raw) } : { ...DEFAULT_SETTINGS };
  } catch(_) { return { ...DEFAULT_SETTINGS }; }
}
function saveSettings(s) { localStorage.setItem('fantasy_settings', JSON.stringify(s)); }

let appSettings = loadSettings();

function applySettings() {
  const root = document.documentElement;
  root.style.setProperty('--item-w', appSettings.itemW + 'px');
  root.style.setProperty('--item-frame', Math.round(appSettings.itemW * 0.88) + 'px');
  root.style.setProperty('--lib-card-w', appSettings.libW + 'px');
  root.style.setProperty('--lib-card-frame', Math.round(appSettings.libW * 0.86) + 'px');
  document.body.style.fontSize = appSettings.fontScale + '%';
  root.classList.toggle('fx-glow', !!appSettings.glow);

  if (appSettings.theme === 'default') root.removeAttribute('data-theme');
  else root.setAttribute('data-theme', appSettings.theme);

  // legacy theme-strip (if still present) + settings grid stay in sync
  document.querySelectorAll('.theme-btn').forEach(b => b.classList.toggle('active', b.dataset.themeId === appSettings.theme));
  document.querySelectorAll('#settingsThemeGrid .style-btn').forEach(b => b.classList.toggle('active', b.dataset.themeId === appSettings.theme));

  // keep the UI controls themselves in sync
  const itemSlider = document.getElementById('itemSizeSlider');
  const libSlider  = document.getElementById('libSizeSlider');
  const fontSlider = document.getElementById('fontScaleSlider');
  const glowToggle = document.getElementById('glowToggle');
  if (itemSlider) { itemSlider.value = appSettings.itemW; document.getElementById('itemSizeVal').textContent = appSettings.itemW + 'px'; }
  if (libSlider)  { libSlider.value  = appSettings.libW;  document.getElementById('libSizeVal').textContent  = appSettings.libW + 'px'; }
  if (fontSlider) { fontSlider.value = appSettings.fontScale; document.getElementById('fontScaleVal').textContent = appSettings.fontScale + '%'; }
  if (glowToggle) glowToggle.checked = !!appSettings.glow;
}
...
function buildSettingsThemeGrid() {
  ...
  ALL_THEMES.forEach(t => {
    const btn = document.createElement('button');
    btn.className = 'style-btn' + (appSettings.theme === t.key ? ' active' : '');
    btn.dataset.themeId = t.key;
    btn.innerHTML = `<span class="style-swatch" style="background:${t.swatch}"></span>${t.label}`;
    btn.addEventListener('click', () => {
      appSettings.theme = t.key;
      saveSettings(appSettings);
      applySettings();
      localStorage.setItem('fantasy_theme', t.key); // keep old key in sync too
    });
...
function initSettingsPanel() {
  ...
  itemSlider?.addEventListener('input', () => {
    appSettings.itemW = parseInt(itemSlider.value, 10);
    document.getElementById('itemSizeVal').textContent = appSettings.itemW + 'px';
    applySettings(); saveSettings(appSettings);
  });
  libSlider?.addEventListener('input', () => {
    appSettings.libW = parseInt(libSlider.value, 10);
    document.getElementById('libSizeVal').textContent = appSettings.libW + 'px';
    applySettings(); saveSettings(appSettings);
  });
  fontSlider?.addEventListener('input', () => {
    appSettings.fontScale = parseInt(fontSlider.value, 10);
    document.getElementById('fontScaleVal').textContent = appSettings.fontScale + '%';
    applySettings(); saveSettings(appSettings);
  });
  glowToggle?.addEventListener('change', () => {
    appSettings.glow = glowToggle.checked;
    applySettings(); saveSettings(appSettings);
  });
...
// Settings sekmesi seçilince paneli tazele
